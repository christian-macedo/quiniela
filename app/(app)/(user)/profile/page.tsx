...
export default function ProfilePage() {
  ...
  const checkPasskeys = useCallback(
    async (userId: string) => {
  ...
  const loadUser = useCallback(async () => {
    ...
    // Layout guarantees authentication; authUser will always be present
    if (!authUser) return;
            ...
            {checkingPasskey ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  />
                  {hasPasskey && (
                    <p className="text-sm text-muted-foreground mt-2">{tPasskeys("addPasskey")}</p>
                  )}
                </div>
              </div>
            )}
